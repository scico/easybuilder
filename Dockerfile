...
USER root
RUN  yum -y install rpm-build bash-completion python-keyring zlib-devel openssl-devel libibverbs-devel unzip
...
RUN curl -O https://raw.githubusercontent.com/hpcugent/easybuild-framework/develop/easybuild/scripts/bootstrap_eb.py  && \
    ...
    module use -a ${EBDIR}/modules/all && module load EasyBuild && eb FPM-1.3.3-Ruby-2.1.6.eb -r 
